...
const actions = {
    ...
    async fetchAllArchiveContracts({commit}, query = null) {
        commit(SET_CONTRACT_ARE_LOADING_MUTATION, true);
        let url = null;
        if (query.page !== null) {
            url = `?page=${query.page}`;
    ...
    async approveContract({commit}, contract) {
        // Vuex actions receive (context, payload) — destructure the file
        // attachment off the payload object instead of relying on a phantom
        // 3rd argument.
        ContractDataService.approve(contract.id, contract.comment, contract.file)
            .then(res => {
    ...
    async requestChangesContract({commit}, payload) {
        try {
            const res = await ContractDataService.requestChanges(
                payload.id, payload.comment, payload.files
            );
            const ok = res && res.data && res.data.data && res.data.data.success;
            commit("contractApproveResponse", ok ? "success" : "error");
            return ok;
        } catch (e) {
            commit("contractApproveResponse", "error");
            return false;
    ...
    async reassignContract({commit}, payload) {
        try {
            const res = await ContractDataService.reassign(
                payload.id, payload.userId, payload.comment
            );
            const ok = res && res.data && res.data.data && res.data.data.success;
            commit("contractApproveResponse", ok ? "success" : "error");
            return ok;
    ...
    async fetchContractTimeline(_ctx, id) {
        try {
            const res = await ContractDataService.timeline(id);
            return (res && res.data && res.data.data && res.data.data.events) || [];
        } catch (e) {
            return [];
        }
    },
    ...
    addSupplierForContractAction({commit}, supplier) {
        commit("setSupplierForContractInputMutation", supplier);
    },
    ...
    editSupplierForContractAction({commit}, supplier) {
        commit("updateSupplierForContractInputMutation", supplier);
    },
...
const mutations = {
    ...
    setSupplierForContractInputMutation: (state, supplier) => {
        state.newContract.supplier = supplier
    },
    ...
    updateSupplierForContractInputMutation: (state, supplier) => {
        state.contract.supplier = supplier
    },
